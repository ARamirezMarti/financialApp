   ...
   AFTER INSERT ON EXPENSES
     FOR EACH ROW
       BEGIN
          ...
          VALUES ((SELECT ACCOUNTS.ACCOUNT_ID FROM EXPENSES inner join ACCOUNTS on EXPENSES.ACCOUNT_ID=ACCOUNTS.ACCOUNT_ID limit 1 ),NEW.EXPENSES_ID,summont, expunt,TB_AMOUNT);
END;
$$
DELIMITER ;
...
DELIMITER $$
CREATE TRIGGER income_BD BEFORE DELETE ON INCOME FOR EACH ROW BEGIN DELETE FROM BALANCE WHERE BALANCE.INCOME_ID=INCOME_ID;
END
$$DELIMITER ;


DELIMITER $$
CREATE TRIGGER expenses_BD BEFORE DELETE ON EXPENSES FOR EACH ROW BEGIN DELETE FROM BALANCE WHERE BALANCE.EXPENSES_ID=EXPENSES_ID;
END
$$DELIMITER ;
